...
- **Global scope**: `/home/vsman/agents-room/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/vsman/agents-room/.claude/notes/`)
...
2. Glob `/home/vsman/agents-room/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/vsman/agents-room/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/vsman/agents-room/.claude/notes/` — works in any directory
