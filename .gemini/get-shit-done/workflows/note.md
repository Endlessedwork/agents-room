...
- **Global scope**: `/home/vsman/agents-room/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/vsman/agents-room/.gemini/notes/`)
...
2. Glob `/home/vsman/agents-room/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (/home/vsman/agents-room/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/vsman/agents-room/.gemini/notes/` — works in any directory
